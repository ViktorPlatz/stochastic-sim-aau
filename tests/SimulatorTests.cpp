...
TEST_CASE("Simulator::runSingleNoGenerator returns full trajectory") {
    ...
    CHECK(result.back().first <= END_TIME + 1); // We add 1 because some reactions may take longer than the end time
}

TEST_CASE("Simulator::runSimulationsConcurrent yields results for multiple sims") {
    int numSim = 3;
    Simulator sim(seihr(100), 42);
...
TEST_CASE("Simulator::runSimulationsNoGenerator returns multiple trajectories") {
    int numSim = 3;
    Simulator sim(seihr(100), 42);
