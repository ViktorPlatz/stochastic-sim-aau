...
TEST_CASE("plotSimulation generates a plot file") {
    ...
    CHECK_NOTHROW(plotSimulation(vessel, result, filename, "SEIHR Plot", excluded));
...
TEST_CASE("plotSimulation throws on empty trajectory") {
    ...
    CHECK_THROWS_AS(plotSimulation(vessel, result, filename, "Empty Plot", excluded), std::runtime_error);
}
